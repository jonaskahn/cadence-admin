  ...
  <UCard class="w-md">
    ...
    <UForm :schema="schema" :state="state" class="flex flex-col gap-4" method="post" @submit="onSubmit">
      <UFormField label="Username" name="username">
        <UInput v-model="state.username" autocomplete="username" class="w-full" placeholder="Enter username" />
      </UFormField>

      <UFormField label="Password" name="password">
        <UInput v-model="state.password" autocomplete="current-password" class="w-full" placeholder="Enter password" type="password" />
      </UFormField>

      <UButton :loading="loading" block label="Sign in" type="submit" />
    </UForm>
  </UCard>
</template>
