  ...
  <div class="w-full max-w-lg flex flex-col gap-6">
    ...
    <div class="flex flex-col gap-3">
      ...
      <UCard v-if="auth.orgList.value.length === 0" class="text-center py-8">
        ...
        <UButton class="mt-2" label="Refresh" size="sm" variant="ghost" @click="auth.loadOrgs()" />
      </UCard>
    </div>

    <div class="text-center">
      <UButton color="neutral" label="Sign out" size="sm" variant="ghost" @click="auth.logout()" />
    </div>
  </div>
</template>
