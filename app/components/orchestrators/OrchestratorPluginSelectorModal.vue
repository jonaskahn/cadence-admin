...
async function fetchPlugins() {
  if (!props.orgId) {
    ...
    pluginsError.value = new Error(t('orchestrators.pluginSelector.selectOrgFirst'))
    plugins.value = []
    return
  }
...
<template>
  <USlideover
    :overlay="true"
    :open="open"
    :title="t('orchestrators.pluginSelector.title')"
    side="top"
    ...
    <template #body>
      <div class="flex flex-col gap-4 h-full">
        <div class="flex flex-wrap gap-2">
          ...
          <UInput v-model="searchQuery" :placeholder="t('orchestrators.pluginSelector.searchPlaceholder')" class="min-w-48" />
        </div>
        ...
        <UAlert v-else-if="pluginsError" color="error" :description="pluginsError.message" :title="t('orchestrators.pluginSelector.failedLoad')" />

        <div v-else class="grid grid-cols-1 gap-4 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          <OrchestratorPluginSelectorCard
        ...
        <p v-if="pluginsStatus === 'success' && filteredPlugins.length === 0" class="py-8 text-center text-dimmed">
          {{ t('orchestrators.pluginSelector.noPluginsMatch') }}
        </p>
      </div>
    </template>
  </USlideover>
