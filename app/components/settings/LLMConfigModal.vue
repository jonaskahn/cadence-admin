...
const providers = ['openai', 'anthropic', 'google', 'groq', 'azure']
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    const msg = (err as { statusMessage?: string })?.statusMessage || 'Failed to add config'
...
<template>
  <UCard class="w-full max-w-md">
    ...
    <UForm :schema="schema" :state="state" class="flex flex-col gap-4" @submit="onSubmit">
      <UFormField label="Config Name" name="name" description="Unique name (e.g. 'primary', 'fast')">
        <UInput v-model="state.name" placeholder="primary" class="w-full" />
      </UFormField>

      <UFormField label="Provider" name="provider">
        <USelect v-model="state.provider" :items="providers" class="w-full" />
      </UFormField>
      ...
      <UFormField label="Base URL" name="base_url" description="Optional custom endpoint">
