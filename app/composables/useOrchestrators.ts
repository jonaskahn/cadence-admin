...
export function useOrchestrators() {
  ...
  async function activate(instanceId: string): Promise<void> {
    try {
      ...
      toast.add({ title: 'Orchestrator activated', icon: 'i-lucide-check' })
    } catch {
      toast.add({ title: 'Failed to activate orchestrator', color: 'error' })
    }
  }
  ...
  async function activatePluginVersion(instanceId: string, pid: string, version: string, source: string): Promise<OrchestratorResponse> {
    ...
    return withErrorToast(async () => {
      ...
      updateOrchestratorInList(orchestrators.value, instanceId, result)
      toast.add({ title: `Plugin ${source}:${pid}@${version} activated`, icon: 'i-lucide-check' })
      return result
