...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    /** Cadence API base URL — server-only; used by server routes (/api/..., OAuth proxy, consent). */
    backendUrl: process.env.NUXT_BACKEND_URL || 'http://localhost:8888',
    /** First-party OAuth2 client id for password/refresh via BFF (must match seeded client). */
    oauthAdminClientId: process.env.NUXT_OAUTH_ADMIN_CLIENT_ID || 'cadence-admin-ui',
    public: {
      appName: process.env.NUXT_PUBLIC_APP_NAME || 'Cadence',
      appTagline: process.env.NUXT_PUBLIC_APP_TAGLINE || 'AI Apps & agent platform',
  ...
  routeRules: {
    '/orchestrators': { redirect: '/ai-apps' },
    '/orchestrators/**': { redirect: '/ai-apps/**' },
    '/settings/orchestrators': { redirect: '/settings/ai-apps' },
    '/plugins': { redirect: '/agent-store' },
    '/plugins/**': { redirect: '/agent-store/**' },
    '/admin/plugins': { redirect: '/admin/agent-store' },
    '/admin/plugins/**': { redirect: '/admin/agent-store/**' }
  },
